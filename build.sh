...
# Simple packaging of orcus-exporter
#
...
binary_artifact="$binary_build_path/bin/orcus-exporter"
...
usage() {
  ...
  echo "-N do not build; use existing ./buld/bin/orcus-exporter binary"
...
setup_build_path() {
  ...
  build_path=$(mktemp -d $release_base_path/orcus-exporterXXXXXX) || fail "Unable to 'mktemp -d $release_base_path/orcus-exporterXXXXXX'"
...
build_binary() {
  ...
  find $binary_artifact -type f || fail "Failed to generate orcus-exporter binary"
}

copy_binary_artifacts() {
  build_path="$1"
  cp $binary_artifact "$build_path/orcus-exporter/usr/bin/" && debug "binary copied to $build_path/orcus-exporter/usr/bin/" || fail "Failed to copy orcus-exporter binary to $build_path/orcus-exporter/usr/bin/"
}
...
setup_artifact_paths() {
  ...
  mkdir -p $build_path/orcus-exporter
  mkdir -p $build_path/orcus-exporter/usr/bin/
  mkdir -p $build_path/orcus-exporter/etc/systemd/system
  mkdir -p $build_path/orcus-exporter/var/log/orcus-exporter
...
function copy_resource_artifacts() {
  ...
  cd  $basedir
  gofmt -s -w  .
  cp etc/systemd/orcus-exporter.service $build_path/orcus-exporter/etc/systemd/system/orcus-exporter.service
...
package_linux() {
  ...
  [ $do_tar -eq 1 ] && tar -C $build_path/orcus-exporter -czf $release_base_path/orcus-exporter-"${RELEASE_VERSION}"-$target-amd64.tar.gz ./

  debug "Creating Distro full packages"
  [ $do_rpm -eq 1 ] && fpm -v "${RELEASE_VERSION}" --epoch 1 -f -s dir -n orcus-exporter -m max-fedotov --description "Orcus Prometheus Exporter" --url "https://github.com/MaxFedotov/orcus-exporter" --vendor "Max Fedotov" --license "Apache 2.0" -C $build_path/orcus-exporter --prefix=/  --rpm-os linux --before-install $build_path/tmp/pre-install -t rpm .
  [ $do_deb -eq 1 ] && fpm -v "${RELEASE_VERSION}" --epoch 1 -f -s dir -n orcus-exporter -m max-fedotov --description "Orcus Prometheus Exporter" --url "https://github.com/MaxFedotov/orcus-exporter" --vendor "Max Fedotov" --license "Apache 2.0" -C $build_path/orcus-exporter --prefix=/  --before-install $build_path/tmp/pre-install -t deb --deb-no-default-config-files .
...
package_darwin() {
  ...
  tar -C $build_path/orcus-exporter -czf $release_base_path/orcus-exporter-"${RELEASE_VERSION}"-$target-amd64.tar.gz ./
}
...
debug "orcus-exporter build done; exit status is $?"
